...
class RenderBlockingResources extends Audit {
  ...
  static get meta() {
    return {
      ...
      title: i18n(UI_STRINGS.title),
      scoreDisplayMode: Audit.SCORING_MODES.NUMERIC,
      description: i18n(UI_STRINGS.description),
  ...
  static async audit(artifacts, context) {
    ...
    let displayValue = '';
    if (results.length > 0) {
      displayValue = i18n(UI_STRINGS.displayValue, {timeInMs: wastedMs, itemCount: results.length});
    }

    /** @type {LH.Result.Audit.OpportunityDetails['headings']} */
    const headings = [
      {key: 'url', valueType: 'url', label: i18n(i18nUtils.UI_STRINGS.columnURL)},
      {key: 'totalBytes', valueType: 'bytes', label: i18n(i18nUtils.UI_STRINGS.columnSize)},
      {key: 'wastedMs', valueType: 'timespanMs', label: i18n(i18nUtils.UI_STRINGS.columnWastedTime)},
    ];
...
module.exports.UI_STRINGS = UI_STRINGS;
